...
<template>
  <div class="apppointment-bg bg-cover-no-repeat h-6235 " id="apppointment-mobile">
    ...
    <div class="flex-items-center-between pl-18 pr-53 pt-8 bg-black h-86 w-full max-w-750 top-0 fixed z-30">
      <img src="/head-logo.png" class="h-70 w-181" />
        <img src="/head-nav.png" class="h-30 w-56 cursor-pointer" @click="setIsShowNav(true)" />
    </div>
    <!-- kv图 -->
    <!-- pt-134 -->
    <div class="kv-bg bg-cover-no-repeat  h-1334 w-full relative" id="main-menu">
      ...
      <img src="/head-title.png" class="ml-66 h-50 w-639 absolute z-1 top-134" />
    ...
    <div id="preorder">
      ...
      <div class="prize-list-bg bg-cover-no-repeat ml-5 mt-42 pt-131 h-501 w-731">
        <div class="ml-47 flex gap-35">
          <img src="/prize-1.png" class="h-236 w-191" />
          <img src="/prize-2.png" class="h-236 w-191" />
      ...
      <div class="appointment-input-bg bg-cover-no-repeat ml-23 mt-20 pt-310 h-557 w-702 relative">
        <img src="/people-talk.png" class="ml-30 h-185 w-649 top-105 absolute z-1" />
        ...
        <div class="flex ml-65 mt-17">
            ...
            <div class="flex radio-bg bg-cover-no-repeat w-28 h-28 font-500 pt-5 pl-3 cursor-pointer" @click="toAgree">
              <img src="/tick.png" class="h-17 w-27 " v-show="isAgree" />
            </div>
          <div
        ...
        <div class="mt-11 ml-218">
            ...
            <!-- v-else -->
            <img src="/appointment-btn1.png" class="h-100 w-277 cursor-pointer" @click="appointment" />
        </div>
      </div>
    </div>
    <!-- 第二部分 -->
        ...
        <div class="time-line bg-cover-no-repeat h-747 w-15 relative ml-90 mt-94  ">
            ...
            <div class="absolute  flex flex-col gap-92 top-[-8%] left-[-38px]">
              <div v-for="item in timeLineArr" :key="item.title" class="  h-90 w-91">
                <img v-if="initData?.reserved_count >= item.num" src="/line-reached.png" class="w-full h-full" />
                <img v-else src="/line-mark.png" class="w-full h-full" />
              </div>
            </div>

        </div>
        <div class="gap-13 flex flex-col ml-55">
          <img v-for="item in timeLineArr" :key="item.title" :src="item.image" class="w-517 h-169" />
        </div>
      </div>
    </div>
    <!-- 第三部分 -->
      ...
      <div class="title-3-bg w-full h-1051 bg-cover-no-repeat pt-33 relative mt-8">
        <div class="step-title-3 bg-cover-no-repeat ml-245 h-105 w-257 relative mb-25">
          ...
          <div class="title-3-decorate bg-cover-no-repeat w-full h-626 relative">
            <div>
              <img :src="`/people/${currentRoleName.name}.png`" class="absolute z-1 "
                :class='currentRoleName.sizeClass' preload />
            </div>
            <img :src="`/people/${currentRoleName.name}-intro.png`"
              class="w-auto h-93 absolute z-2 bottom-6% right-1%" />
            <img :src="`/people/${currentRoleName.name}-symbel.png`"
          ...
          <div class="relative h-307">
            <div class="ml-51 mr-39 flex gap-5 mt-30  overflow-auto pb-12 absolute z-2 " ref="roleListRef">
              <div v-for="item in rolesList" :key="item.name" class="flex-shrink-0  cursor-pointer flex-col flex"
                ...
                <img :src="`/roles/${item.name}-selected.png`" v-show="item.isSelected" class="w-128 h-184" />
                <img :src="`/roles/${item.name}.png`" v-show="!item.isSelected" class="w-128 h-184" />
                <img v-show="item.isSelected" src="/line-selected.png" class="w-35 h-2 mt-15  ml-33" />
              </div>
            </div>
    ...
    <div class="mt-30" id="game-features">
      <div class="step-title-4 bg-cover-no-repeat ml-265 h-105 w-221 relative">
        ...
        <div class="swiper-bg w-579 h-903 relative ml-83 bg-cover-no-repeat  pt-81">
          <img src="/swiper-left.png" class="w-52 h-78 absolute top-45% left-[-6%] cursor-pointer"
            @click="swiperPrev" />
          <img src="/swiper-right.png" class="w-52 h-78 absolute top-45% right-[-6%] cursor-pointer"
            @click="swiperNext" />
    ...
    <!-- 导航弹窗 -->
      <van-popup v-model:show="isShowNav" class='h-full !bg-transparent !w-full !max-w-750 ' z-index="40"
        ...
        <div class='text-right pt-5 pr-40 flex flex-row-reverse'>
          <img src="/popup/close.png" class="w-84 h-85 cursor-pointer" @click="setIsShowNav(false)" />
        </div>
        <div class="ml-138">
          <img src="/popup/nav-icon.png" class="w-481 h-188 " />
        </div>
        <div class="flex flex-col items-center font-500 font-[NotoSansSC] text-[48px]  text-[#fff] pt-71">
          <div class="cursor-pointer relative " v-for="item in navList" :key="item.name"
            ...
            <img v-show="item.isHover || activeNav === item.image" :src="`/popup/${item.image}.png`"
              class="w-184 h-59  " />
        ...
        <div class="mt-73 pl-79">
          <img src="/popup/nav-line.png" class="w-583 h-12 " />
        </div>
        <div class="flex ml-219 pt-24">
          <img src="/popup/line.png" class="w-143 h-115 mr-26 mt-7 cursor-pointer" />
          <img src="/popup/youtube.png" class="w-144 h-126 cursor-pointer" />
        </div>
      </van-popup>
    <!-- 预约弹窗(商店和手机) -->
