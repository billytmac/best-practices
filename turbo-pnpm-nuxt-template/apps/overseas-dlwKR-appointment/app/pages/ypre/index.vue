...
import useYpreCommon from '~/composables/useYpreCommon'


const { changeRole, currentPopupRoleName, popupRolesArr, bannerArr, getImageUrl, initOperation, storagePhoneInfo, openUrl, reservationInitApi, isIos, isAlreadyAppointment, isGoShop, openStoreUrl, toBindOs, toAgree, appointment, roleListRef, activeNav, isShowTipPopup, tipText, initData, showBottomPopup, isShowAppointmentPopup, isShowPhoneAppointmentPopup, isShowPhoneAppointmentSuccessPopup, isShowAppointmentSuccessPopup, isShowAnnouncementsPopup, bindOs, isAgree, inputValue } = useYpreCommon()
...
<template>
  <div class="apppointment-bg bg-cover-no-repeat h-6235 " id="apppointment-mobile">
    ...
    <div class="kv-bg bg-cover-no-repeat  h-1334 w-full relative " id="main-menu">
      ...
      <!-- 明星版 -->
      <img src="/assets/images/play.png" class="ml-66 h-120 w-122 absolute z-1 top-983 mx-auto left-245"
        @click="playVideo" />
    ...
    <div id="preorder">
      ...
      <div class="appointment-input-bg bg-cover-no-repeat ml-23 mt-20 pt-250 h-557 w-702 relative">
        <div class=" flex-col-items-center">
          <img src="/assets/images/people-talk.png" class=" h-185 w-649 top-[17%] absolute" />
          <div class="flex cursor-pointer gap-10 mt--13 relative">
            <div v-for="item in popupRolesArr" :key="item" class=" w-74 h-115 " @click="changeRole(item)">
              <img :src="getImageUrl(item + '-selected', 'ypre')" class="w-full h-full"
                v-show="currentPopupRoleName === item" />
              <img :src="getImageUrl(item, 'ypre')" class="w-full h-full" v-show="currentPopupRoleName !== item" />
            </div>
          </div>
          <div
            class="download-bg bg-cover-no-repeat h-80 w-621  pl-52 font-500 text-[#5C47DD] flex-items-center font-[NotoSansSC] mt-8">
          ...
          <div class=" mx-auto h-81 w-401">
            <img src="/assets/images/ypre/appointment-btn1.png" class="h-full w-full cursor-pointer"
              @click="appointment" />
          </div>
        </div>
    ...
    <van-popup v-model:show="isShowNav" class='h-full !bg-transparent !w-full !max-w-750 ' z-index="40"
      ...
      <div class="flex ml-219 pt-24">
        <img @click="openUrl('loungeUrl')" src="/assets/images/popup/line.png"
    ...
    <van-popup v-model:show="isShowAppointmentPopup"
      class='h-full !bg-transparent !w-full !max-w-750 flex-col-items-center-center' z-index="40"
      overlay-class='!bg-black/80' :close-on-click-overlay="false">
      <div class="appointment-popup-bg1 bg-cover-no-repeat h-1223 w-679  pt-300 flex-col-items-center">
        <div class="  relative">
          <img src="/assets/images/ypre/appointment-title.png" class="w-615 h-94" />
          <img src="/assets/images/popup/close.png" class="w-84 h-85 absolute right-[-8.7%] top-[-13%] cursor-pointer"
            @click="isShowAppointmentPopup = false" />
        </div>
        <div class=' mt-42 '>
          <img src="/assets/images/ypre/step-1.png" class="w-479 h-50" />
        </div>
        ...
        <div class="mt-13  w-215 h-63 appointmentPopup-store-wrap shine-wrapper overflow-hidden relative"
          @click="openStoreUrl('phoneAndShopPopup')">
          <img v-show='isIos' src="/assets/images/ypre/apple-appointment.png" class="w-full h-full" />
          <img v-show="!isIos" src="/assets/images/ypre/google-appointment.png" class="w-full h-full" />
        </div>
        <div class=' mt-4'>
          <img src="/assets/images/ypre/step-2.png" class="w-479 h-50" />
        </div>
        <div class="flex cursor-pointer mt-20">
          <div v-for="item in popupRolesArr" :key="item" class=" w-121 h-181 " @click="changeRole(item)">
            <img :src="getImageUrl(item + '-selected', 'ypre')" class="w-full h-full"
              v-show="currentPopupRoleName === item" />
            <img :src="getImageUrl(item, 'ypre')" class="w-full h-full" v-show="currentPopupRoleName !== item" />
          </div>
    ...
    <van-popup v-model:show="isShowPhoneAppointmentPopup"
      ...
      overlay-class='!bg-black/80' :close-on-click-overlay="false">
      <div class="relative ">
        <img src="/assets/images/ypre/phone-title.png" class="w-636 h-94" />
        <img src="/assets/images/popup/close.png" class="w-84 h-85 absolute  cursor-pointer right-[-6.7%] top-[-10%]"
          @click="isShowPhoneAppointmentPopup = false" />
      </div>
      <div class="phone-appointment-popup-bg bg-cover-no-repeat h-834 w-655 mt-13 pt-54 flex-col-items-center">
        <!-- <div class="w-333 h-165">
          <img src="/assets/images/popup/phone-info.png" class="w-full h-full" />
        </div>
        <div class="mt-4 ">
          <img src="/assets/images/popup/phone-dialog.png" class="w-339 h-117" />
        </div> -->
        <!-- <div class="text-[26px] text-white leading-[26px] font-500 flex  mt-85">
        ...
        <div class="flex cursor-pointer mt-370">
          <div v-for="item in popupRolesArr" :key="item" class=" w-121 h-181 " @click="changeRole(item)">
            <img :src="getImageUrl(item + '-selected', 'ypre')" class="w-full h-full"
              v-show="currentPopupRoleName === item" />
            <img :src="getImageUrl(item, 'ypre')" class="w-full h-full" v-show="currentPopupRoleName !== item" />
    ...
    <!-- 电话预约成功,商店没点击时弹窗 -->
    <van-popup v-model:show="isShowPhoneAppointmentSuccessPopup"
      class='h-full !bg-transparent !w-full !max-w-750 flex-col-items-center-center' z-index="40"
      overlay-class='!bg-black/80' :close-on-click-overlay="false">

      <div class="phone-appointment-success-popup-bg bg-cover-no-repeat h-986 w-666  pt-295 flex-col-items-center">
        <div class="relative">
          <img src="/assets/images/ypre/phone-success-title.png" class="w-636 h-94" />
          <img src="/assets/images/popup/close.png" class="w-84 h-85 absolute right--6.4% top--9% cursor-pointer"
            @click="isShowPhoneAppointmentSuccessPopup = false" />
        </div>
        <img src="/assets/images/popup/phone-go-market-title.png" class="w-564 h-42 mt-74 " />
        <div class=' mt-42 flex'>
        ...
        <div class="w-300 h-89 mt-47  phone-store-wrap shine-wrapper overflow-hidden relative"
    ...
    <!-- 预约成功弹窗 -->
    <van-popup v-model:show="isShowAppointmentSuccessPopup"
      class='h-full !bg-transparent !w-full !max-w-750 flex-col-items-center-center' z-index="40"
      overlay-class='!bg-black/80' :close-on-click-overlay="false">
      <div class="flex">
      ...
      <div class="success-popup-bg bg-cover-no-repeat h-562 w-657 mt-9 pt-20 flex-col-items-center">
        <div class="">
          <img src="/assets/images/ypre/success-icon.png" class="w-502 h-396" />
        </div>
...
.appointment-popup-bg1 {
  background-image: url("/assets/images/ypre/appointment-bg1.png");
}

.phone-appointment-popup-bg {
  background-image: url("/assets/images/ypre/phone-bg.png");
}

.phone-appointment-success-popup-bg {
  background-image: url("/assets/images/ypre/phone-success-bg.png");
}
...
.success-popup-bg {
  background-image: url("/assets/images/ypre/success-bg1.png");
}
